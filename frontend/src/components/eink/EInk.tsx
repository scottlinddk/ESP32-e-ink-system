...
function drawNews(ctx: CanvasRenderingContext2D, x: number, y: number) {
  ctx.strokeStyle = '#000';
  ctx.lineWidth = 1;
  ctx.fillStyle = '#000';
...
function renderEink(ctx: CanvasRenderingContext2D, opts: RenderOpts) {
  ...
  if (sources.energy) enabled.push('energy');
  if (sources.weather) enabled.push('weather');
  if (sources.news) enabled.push('news');
  ...
  if (sources.news) {
    if (!keys.news) {
      ...
      lines
        .slice(0, 2)
        .forEach((ln, i) => ctx.fillText(ln, PAD + 20, y + 9 + i * 11));
    }
  }
}
