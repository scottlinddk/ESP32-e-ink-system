...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const userId = await resolveUserId(req.clerkUserId!);
      const { device_id, device_name, ble_name } = req.body as {
        device_id?: string;
        device_name?: string;
        ble_name?: string;
      };
      ...
      // device_id defaults to the BLE name (OD prefix + chip ID) when provided
      const hardwareId =
        (device_id ?? ble_name ?? '').trim() ||
        'OD-' + crypto.randomBytes(3).toString('hex').toUpperCase();
